...
public:
                            ...
                            pcl::PointCloud<pcl::PointXYZ>::Ptr& clusteredCloud) 
    {
        ...
        if (inputCloud->empty()) {
            ROS_WARN("Input cloud is empty. Skipping clustering.");
            return;
        }
    ...
    if (!clusteredCloud->empty()) {
        ...
        // Calculate the angle of the point using atan2
        float angle = atan2(centerPoint.y, centerPoint.x);
        ...
        // Calculate the range value using sin(theta)
